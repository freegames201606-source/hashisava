...
/* このゲーム中に撃破したか（保存用） */
let killedRedeyeThisGame = false;
let killedQueenbeeThisGame = false;
...
function resetGame() {
  ...
  killedRedeyeThisGame = false;
  killedQueenbeeThisGame = false;
  gameSpeedMul = 1;
  if (speedBar) {
    const buttons = speedBar.querySelectorAll('.speed-btn');
    buttons.forEach(b => b.classList.remove('selected'));
...
function update(dt) {
  ...
  if (player.hp <= 0) {
    ...
    /* 撃破カウントを保存 */
    if (killedRedeyeThisGame) {
      redeyeKills++;
      saveKills(KILL_KEY_RED, redeyeKills);
    }
    if (killedQueenbeeThisGame) {
      queenbeeKills++;
